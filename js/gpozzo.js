...
var categorias = [{ label: 'Poda y limpieza', value: 'poda', estatica: true} ,{ label: 'Iluminacion Erronea', value: 'iluminacion', estatica: false} ,{ label: 'Falta de camaras', value: 'camaras', estatica: false} ,{ label: 'Inseguridad', value: 'inseguridad', estatica: false} ,{ label: 'Bacheo', value: 'bacheo', estatica: false} ,{ label: 'Otro', value: 'otro', estatica: false} ];
...
function setStage() {
  ...
  //Extras
    ...
    //Zona
    if("jctrEnzubJlwBpsC`QdIb{@~~@p~@``AnxBhdChk@eb@fbDssBdEtIxa@ub@zo@_`@fvAoaAkVcPum@s@}FaGm`@uAc{AohDl]iYqlFcbMdA~MuLr@cc@t^oBpG}JNgHfIrArO_JeIw^hZqg@paAcMiC`FhKkRpd@}FvIwW}LvW`VaJ?gSyQhKt^".length){
  var path = new google.maps.Polygon({
    ...
    fillOpacity: 0.25,
    map: mapa,
    paths: google.maps.geometry.encoding.decodePath("jctrEnzubJlwBpsC`QdIb{@~~@p~@``AnxBhdChk@eb@fbDssBdEtIxa@ub@zo@_`@fvAoaAkVcPum@s@}FaGm`@uAc{AohDl]iYqlFcbMdA~MuLr@cc@t^oBpG}JNgHfIrArO_JeIw^hZqg@paAcMiC`FhKkRpd@}FvIwW}LvW`VaJ?gSyQhKt^")
  });
}
...
function loadMarker(location, loadedMarker) {
  ...
  //Nunjucks: opciones.markerSolucionable
      ...
      google.maps.event.addListener(marker, 'rightclick', function() {
            ...
            //Max markers remove
            if (marker.solucionado == false && cant_solucionados >= 2) {
  ...
  infowindow.close();
  //Nunjucks: opciones.infowindow
  if (marker.texto) infowindow.setContent('<h4><small>' + 'ID: '+marker.id + ' - ' +'Agregado: '+marker.fecha  + '</small><br>' + marker.texto + '</h4>' + '' + '<a onclick="comoLlegar('+marker.position.lat()+','+marker.position.lng()+')" class="btn btn-block btn-primary"><span class="fa fa-map-marker fa-lg"></span> Como llegar</a>' + '');
  else infowindow.setContent('<h4><small>' + 'ID: '+marker.id + ' - ' +'Agregado: '+marker.fecha  + '</small></h4>' + '' + '<a onclick="comoLlegar('+marker.position.lat()+','+marker.position.lng()+')" class="btn btn-block btn-primary"><span class="fa fa-map-marker fa-lg"></span> Como llegar</a>' + '');
